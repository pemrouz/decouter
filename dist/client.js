...
var router = function router(routes) {
  ...
  function route(req, res, next) {
    ...
    return from !== to ? (go(to), resolved) : resolved;
  }
};
...
var next = function next(req) {
  ...
  return is.promise(value) ? value.then(function (v) {
    return next(req, params, url, v, variable);
  }) : is.str(value) || is.bol(value) ? value : is.fn(value) && !is.def(variable) ? next(req, params, url, value(req)) : is.fn(value) ? next(req, params, url, value(variable, req)) : cur in value ? next(req, params, remainder, value[cur]) : !cur && value[':'] ? next(req, params, remainder, value[':']) : key('value')(variables(value).find(function (d) {
...
var variables = function variables(routes) {
  return keys(routes).filter(function (_ref) {
